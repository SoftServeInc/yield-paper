...
def run_training(args: Namespace, logger: Logger = None) -> List[float]:
    ...
    if args.separate_val_path and args.separate_test_path:
        ...
        train_data, _, test_data = split_data(data=data, split_type=args.split_type, sizes=args.split_sizes, seed=args.seed, args=args, logger=logger)
    elif args.separate_test_path:
        assert args.split_sizes[2] == 0.0
        train_data, val_data, _ = split_data(data=data, split_type=args.split_type, sizes=args.split_sizes, seed=args.seed, args=args, logger=logger)
    else:
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for epoch in trange(args.epochs):
            ...
            if args.show_individual_scores:
                ...
                for task_name, val_score in zip(args.task_names, val_scores):
                    ...
                    vwriter.add_scalar(f'validation_{task_name}_{args.metric}', val_score, n_iter)

            # Save model checkpoint if improved validation score
            val_score = val_scores[0] if args.validate_on_first_target else avg_val_score
        ...
        if args.show_individual_scores:
            ...
            for task_name, test_score in zip(args.task_names, test_scores):
                ...
                vwriter.add_scalar(f'test_{task_name}_{args.metric}', test_score, n_iter)
    ...
    vwriter.add_scalar(f'ensemble_test_{args.metric}', avg_ensemble_test_score, 0)
